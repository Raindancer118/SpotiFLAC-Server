...
export function DragDropMedia({ value, onChange, className }: DragDropMediaProps) {
    ...
    const handleFiles = async (fileList: File[]) => {
        ...
        for (let i = 0; i < fileList.length; i++) {
            ...
            try {
                ...
                const result = await apiClient.UploadImageBytes(new Uint8Array(), file.name);
    ...
    const handleSelectFile = async () => {
        try {
            const paths = await apiClient.SelectImageVideo();
            if (paths && paths.length > 0) {
                const timestamp = Date.now();
                const newFiles: UploadedFile[] = paths.map((p: string, i: number) => ({
                ...
                for (let i = 0; i < paths.length; i++) {
                    ...
                    const fileId = newFiles[i].id;
                    try {
                        const result = await apiClient.UploadImage(path);
                        setFiles(prev => prev.map(f => f.id === fileId
                            ? { ...f, status: 'done', url: result }
                            : f));
                    }
